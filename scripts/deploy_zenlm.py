#!/usr/bin/env python3
"""
Deploy Supra Nexus o1 models to HuggingFace under zenlm namespace.
Includes comprehensive format support and professional model cards.
"""

import subprocess
from pathlib import Path

def create_supra_model_card(model_name: str, is_thinking: bool = False) -> str:
    """Create comprehensive model card for Supra models under zenlm."""
...
wget https://huggingface.co/zenlm/{model_name.lower()}/resolve/main/{model_name.lower().replace(' ', '-')}-q4_k_m.gguf
...
- [supra-nexus-o1-thinking](https://huggingface.co/zenlm/supra-nexus-o1-thinking) - Chain-of-thought reasoning
- [supra-nexus-o1-instruct](https://huggingface.co/zenlm/supra-nexus-o1-instruct) - Direct instruction following
- [supra-identity dataset](https://huggingface.co/datasets/zenlm/supra-identity) - Training data
...
def upload_supra_model(model_path: str, repo_name: str, model_card: str) -> bool:
    """Upload Supra model to zenlm namespace."""
    ...
    try:
        ...
        # Create repo first  
        subprocess.run(f"hf repo create zenlm/{repo_name}", shell=True, check=False)
        
        # Upload model files
        cmd = f"hf upload zenlm/{repo_name} {model_path}"
        subprocess.run(cmd, shell=True, check=True)
        
        print(f"✅ {repo_name} uploaded successfully")
        print(f"🔗 https://huggingface.co/zenlm/{repo_name}")
        return True
...
def upload_supra_dataset() -> bool:
    """Upload Supra identity dataset to zenlm."""
    
    print("📊 Uploading Supra identity dataset...")
    
    try:
        # Create dataset repo
        subprocess.run("hf repo create zenlm/supra-identity", shell=True, check=False)
        
        # Upload dataset files
        subprocess.run("hf upload zenlm/supra-identity supra_identity_data.jsonl", shell=True, check=True)
        subprocess.run("hf upload zenlm/supra-identity supra_dataset_README.md", shell=True, check=True)
        
        print("✅ Dataset uploaded successfully")
        print("🔗 https://huggingface.co/datasets/zenlm/supra-identity")
        return True
...
def main():
    """Deploy Supra Nexus o1 ecosystem to zenlm namespace."""
    
    print("🎯 Deploying Supra Nexus o1 Ecosystem to HuggingFace (zenlm)")
    print("=" * 65)
    ...
    for model in models:
        ...
        model_card = create_supra_model_card(
            model["name"],
            model["thinking"]
        )
        
        if upload_supra_model(model["path"], model["repo"], model_card):
            success_count += 1
    ...
    if success_count == len(models) + 1:  # +1 for dataset
        ...
        print("✅ Professional branding with Supra Foundation LLC identity")
        print("✅ MLX + GGUF + Transformers support documented")
        
        print("\\n🔗 Live ecosystem:")
        print("• Thinking model: https://huggingface.co/zenlm/supra-nexus-o1-thinking")
        print("• Instruct model: https://huggingface.co/zenlm/supra-nexus-o1-instruct")
        print("• Identity dataset: https://huggingface.co/datasets/zenlm/supra-identity")
        ...
        print("\\n🏢 Supra Foundation LLC branding maintained")
        print("📅 2025 dating throughout ecosystem")
